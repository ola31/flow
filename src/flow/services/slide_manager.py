...
class PPTLoadWorker(QThread):
    ...
    def run(self):
        try:
            count = self.manager._do_load_pptx(
                self.path, progress_callback=self._emit_progress
            )
...
class SlideManager(QObject):
                ...
                if self._slide_count > 0:
                    for i in range(self._slide_count):
                        ...
                        if (i + 1) % 5 == 0 or i + 1 == self._slide_count:
                            print(
                                f"[SlideManager] 이미지 생성 중... ({i + 1}/{self._slide_count})"
                            )

                elapsed = time.time() - start_time
                print(
                    f"[SlideManager] PPT 로드 완료: {self._slide_count} 슬라이드 전체 변환됨 (총 소요 시간: {elapsed:.2f}초)"
                )
    ...
    def reload_song(self, song) -> int:
        """단일 곡의 PPT를 다시 로드 (캐시 무효화 및 시그널 발생)"""
        if not song.has_slides:
            return 0

        abs_path = song.abs_slides_path

        # 로딩 시작 알림
        self.load_started.emit()
        engine_name = self._converter.get_engine_name()

        # 1. 해당 곡의 캐시 무효화
        self._converter.invalidate_cache(abs_path)

        # 2. 슬라이드 개수 다시 확인
        try:
            prs = Presentation(str(abs_path))
            new_count = len(prs.slides)
            old_count = song.get_slide_count()
            song.set_slide_count(new_count)

            # 3. 슬라이드 개수 변경 시 전체 오프셋 재계산
            if new_count != old_count:
                self._recalculate_offsets()

            # 4. 새 슬라이드 이미지 미리 생성 및 보고
            for i in range(new_count):
                self._converter.convert_slide(abs_path, i)
                self.load_progress.emit(i + 1, new_count, engine_name)

            print(f"[SlideManager] 곡 리로드 완료: {song.name} ({new_count}장)")
            self.load_finished.emit(self._total_slide_count)
            return new_count

        except Exception as e:
            print(f"⚠️ 곡 리로드 실패: {song.name} - {e}")
            return 0

    def reload_all_songs(self) -> int:
        """모든 곡의 PPT를 다시 로드 (비동기 시뮬레이션 및 시그널 발생)"""
        # 1. 전체 캐시 무효화
        ...
        # 3. 각 곡의 슬라이드 미리 생성 및 진행률 보고
        self._slide_offsets = {}
        offset = 0
        engine_name = self._converter.get_engine_name()

        # 전체 슬라이드 수 미리 파악 (진행률 계산용)
        total_count = 0
        for song in self._songs:
            if song.has_slides:
                try:
                    prs = Presentation(str(song.abs_slides_path))
            ...
            if song.has_slides:
                ...
                try:
                    ...
                    for i in range(count):
                        self._converter.convert_slide(abs_p, i)
                        current_progress += 1
                        self.load_progress.emit(
                            current_progress, total_count, engine_name
                        )
