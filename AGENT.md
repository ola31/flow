...
- **Enter**: 현재 Preview(대기) 상태의 슬라이드를 Live(송출)로 보냅니다.
- **1~6**: 송출 중인 곡의 절(Verse)을 변경합니다. (1~4=1~4절, 5=5절, 6=후렴)
- **Arrow Keys (Up/Down)**: 핫스팟 목록 내에서 이전/다음 항목을 순회하며 'Preview' 상태만 업데이트합니다.
- **Arrow Keys (Left/Right)**: 곡(Sheet) 간 이동을 수행합니다.
- **포커스 정책**: 자식 위젯이 포커스를 가지고 있더라도 위 키들은 반드시 `MainWindow`에서 가로채어 의도된 동작을 수행해야 합니다.
...
- **로직**: `Hotspot.get_slide_index(5)`는 내비게이션 실패 시의 안전장치로 자주 호출됩니다. 이를 임의로 수정하면 흐름이 깨집니다.
